...
- **Verificación en dos niveles**:
  1. **Identidad**: el técnico sube su INE *y* un comprobante de domicilio juntos; el admin los aprueba o rechaza.
  2. **Habilidades por categoría**: el técnico sube una foto que demuestre su experiencia en Plomería, Electricidad, Carpintería, Cerrajería, Pintura, Limpieza o A/C — no es obligatorio verificar todas. **Un técnico solo ve y puede ofertar en solicitudes de las categorías donde ya tiene la habilidad aprobada** — esto se aplica a nivel de base de datos (Row Level Security), no solo visualmente.
- **Mapa gratuito (Leaflet + OpenStreetMap)**: sin API key, sin tarjeta de crédito. El cliente puede usar el botón "📍 Mi ubicación" (geolocalización del navegador) al confirmar una solicitud, y esa ubicación se muestra como un mini-mapa tanto en "Tus solicitudes" como en el panel del técnico.
- Costo de visita configurable, panel de Admin con técnicos reales y ventas reales.
...
> **Nota:** vuelve a pegar `supabase/schema.sql` completo en el SQL Editor y dale **Run** — es seguro, no borra tus datos. Esta vez agrega dos tablas nuevas (`service_offers`, `skill_verifications`), una columna nueva importante (`profiles.verified_specialties`), y cambia la regla de quién ve qué solicitud — no te lo saltes.
