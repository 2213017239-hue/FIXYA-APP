...
- Directorio de técnicos con foto, zona, especialidades y verificación reales
- Portafolio de trabajos anteriores
- **Sistema de ofertas**: cuando un cliente publica una solicitud, *varios* técnicos pueden enviar su propio precio. El cliente ve todas las ofertas recibidas y elige con cuál técnico trabajar — o le puede proponer un precio menor a un técnico específico, quien puede aceptarlo o mantener el suyo.
- **Costo de visita**: cada solicitud aclara que primero se cobra una visita de diagnóstico (configurable, $150 MXN por defecto en `app.js`), y que el precio final del trabajo se acuerda en persona.
- **El simulador ya no pide cuenta de entrada**: cualquiera puede elegir categoría, ver el rango de precio y escribir su problema. Solo al intentar escribir la **dirección exacta** se le pide iniciar sesión o crear cuenta.
- Panel de técnico con Mi Perfil (foto, zona, especialidades), palomita de verificado junto a su propia foto, y "Mis Ofertas" para dar seguimiento a cada precio que envió.
- Panel de Admin con lista real de todos los técnicos registrados, verificación de identidad, y ventas reales.
- Asistente de preguntas frecuentes (no es IA generativa — ver más abajo).
...
> **Nota:** vuelve a pegar `supabase/schema.sql` completo en el SQL Editor y dale **Run** — es seguro, no borra tus datos ni tablas existentes. Esta vez agrega una tabla nueva (`service_offers`) y cambia cómo funcionan las políticas de "quién puede aceptar qué", así que es un paso importante no saltarse.
