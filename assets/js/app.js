...
function ratingBadgeHTML(t){
  ...
  return `<span class="tech-rating" style="color:var(--muted);">Nuevo en FixYa</span>`;
}
...
myRequestsList?.addEventListener('click', async (e) => {
  ...
  if (btn.classList.contains('js-whatsapp-tech')) {
    ...
    openWhatsApp(tech?.phone, `Hola, soy tu cliente en FixYa para el servicio de ${r.category}. ¿Cómo vamos?`);
    return;
  }
...
myOffersList?.addEventListener('click', async (e) => {
  ...
  if (btn.classList.contains('js-whatsapp-client')) {
    openWhatsApp(btn.dataset.phone, `Hola, soy tu técnico de FixYa para tu solicitud de ${btn.dataset.cat}. ¿Cuándo te viene bien la visita?`);
    return;
  }
  const offerId = btn.dataset.offer;
