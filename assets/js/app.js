...
function myRequestCardHTML(r){
  ...
  if (r.status === 'confirmada') {
    ...
    body = `<p style="font-size:.85rem;color:var(--accent);font-weight:700;">✓ Trabajando con ${r.tech_name || 'tu técnico'} — Precio acordado: ${fmtMXN(r.final_price || 0)}</p>`;
  } else {
    body = `<p style="font-size:.82rem;color:var(--muted);">${r.status === 'completada' ? 'Servicio completado.' : 'Solicitud cancelada.'}</p>`;
  }
...
async function loadMyRequests(){
  ...
  const { data, error } = await supabase
    ...
    .select('*, service_offers(*, profiles(name, verified))')
  ...
  // Para las solicitudes ya asignadas, busca el nombre del técnico elegido.
  const withTechName = await Promise.all(rows.map(async (r) => {
    if (r.status === 'aceptada' && r.technician_id) {
      const { data: tech } = await supabase.from('profiles').select('name').eq('id', r.technician_id).single();
      r.tech_name = tech?.name;
    }
    return r;
  }));

  myRequestsList.innerHTML = withTechName.map(myRequestCardHTML).join('');
  withTechName.forEach(r => { if (r.latitude != null) renderMiniMap(`myReqMap-${r.id}`, r.latitude, r.longitude); });
}
...
function myOfferCardHTML(o){
  ...
  if (o.status === 'pendiente') {
    ...
    statusHTML = `<div class="offer-status won">✓ El cliente te eligió</div>`;
  } else {
...
async function loadMyOffers(){
  ...
  const { data, error } = await supabase
    .from('service_offers')
    .select('*, service_requests(category, address, status)')
    .eq('technician_id', user.id)
    .order('created_at', { ascending: false });
  if (error) { console.error('[FixYa] Error al leer tus ofertas:', error.message); return; }
...
async function loadSkillVerifications(){
  ...
  if (error) { console.error('[FixYa] Error al leer habilidades:', error.message); return; }
...
async function loadAdminTechnicians(){
  ...
  if (error) { console.error('[FixYa] Error al leer técnicos:', error.message); return; }
  const rows = data || [];
