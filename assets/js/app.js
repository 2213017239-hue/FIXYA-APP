...
function openGuaranteeModal(data) {
  ...
  modal.innerHTML = `
    <div class="modal-card" style="max-width:600px; padding:2rem; border:2px solid var(--accent); position:relative; background:var(--surface);">
      <button type="button" class="modal-close" id="closeGuaranteeBtn" style="position:absolute; top:1rem; right:1rem; font-size:1.5rem; background:none; border:none; color:var(--text); cursor:pointer;">&times;</button>
      <div style="text-align:center; border-bottom:2px dashed var(--border); padding-bottom:1.2rem; margin-bottom:1.2rem;">
        <div style="font-size:2.5rem; margin-bottom:0.3rem;">🛡️</div>
        <h2 style="margin:0; font-size:1.5rem; color:var(--accent);">CERTIFICADO DE GARANTÍA DIGITAL</h2>
        <span style="font-size:0.85rem; color:var(--muted); font-weight:700;">OFICIOS 60 MINUTOS — RESPALDO EN MANO DE OBRA</span>
      </div>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; font-size:0.9rem; margin-bottom:1.5rem;">
        <div>
          <span style="color:var(--muted); font-size:0.8rem;">FOLIO DE GARANTÍA</span>
          <strong style="display:block; font-family:monospace; font-size:1.1rem; color:var(--text);">${folio}</strong>
        </div>
        <div>
          <span style="color:var(--muted); font-size:0.8rem;">COBERTURA</span>
          <strong style="display:block; color:var(--accent2);">30 Días Naturales</strong>
        </div>
        <div>
          <span style="color:var(--muted); font-size:0.8rem;">TÉCNICO ESPECIALISTA</span>
          <strong style="display:block; color:var(--text);">${data.tech || 'Especialista Verificado'}</strong>
        </div>
        <div>
          <span style="color:var(--muted); font-size:0.8rem;">SERVICIO REALIZADO</span>
          <strong style="display:block; color:var(--text);">${data.cat || 'Servicio General'} (${data.date || ''})</strong>
        </div>
      </div>
      <div style="background:var(--surface-2); padding:1rem; border-radius:0.6rem; font-size:0.82rem; color:var(--text); line-height:1.4; margin-bottom:1.5rem;">
      ...
      <div style="display:flex; justify-content:flex-end; gap:0.8rem;">
        <button type="button" class="btn btn-secondary btn-sm" onclick="window.print()">🖨️ Imprimir / PDF</button>
        <button type="button" class="btn btn-primary btn-sm" id="closeGuaranteeBtn2">Cerrar</button>
      </div>
    </div>
  `;
...
async function loadTechnicianStats(){
  ...
  provEarningsSub.textContent = thisMonth.length > 0
    ? `Neto de ${thisMonth.length} servicio${thisMonth.length === 1 ? '' : 's'} (comisión de Oficios60minutos ya descontada)`
